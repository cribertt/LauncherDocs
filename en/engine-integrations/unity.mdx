...
<Info>
  You can find the download link through our Discord server. Open the **Download plugin** section and select the channel for **Unity**.
</Info>
...
<CardGroup cols={2}>
  <Card title="LauncherForge Launch Authentication 1.0.2" icon="download">
    ...
    Download the package from the Unity channel inside **Download plugin**.
  </Card>

  <Card title="UnityThirdPersonAuthTest" icon="flask">
    <Badge color="gray">Not available yet</Badge>

    Sample project for Unity 6 or another package-compatible version.
  </Card>
</CardGroup>

<Note>
  The package is distributed as `LauncherForge_LauncherAuth_Unity_1.0.2.zip`. The sample project is identified as `UnityThirdPersonAuthTest`.
</Note>
...
<Card title="UnityThirdPersonAuthTest" icon="flask">
  ...
  The sample project is intended for Unity 6 or another package-compatible version. Its download link does not yet exist in the source document.
</Card>
