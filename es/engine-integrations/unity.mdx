...
<Info>
  El flujo es automático. No necesitas añadir el package a un `GameObject` ni crear un bootstrap manual.
</Info>
...
<Info>
  Puedes encontrar el enlace de descarga a través de nuestro Discord. Abre la sección **Download plugin** y busca el canal correspondiente a **Unity**.
</Info>

<CardGroup cols={2}>
  <Card title="LauncherForge Launch Authentication 1.0.2" icon="download">
    <Badge color="green">Disponible en Discord</Badge>

    Descarga el package desde el canal de Unity dentro de **Download plugin**.
  </Card>

  <Card title="UnityThirdPersonAuthTest" icon="flask">
    <Badge color="gray">No disponible por el momento</Badge>

    Proyecto de ejemplo para Unity 6 o una versión compatible con el package.
  </Card>
</CardGroup>

<Note>
  El package se distribuye como `LauncherForge_LauncherAuth_Unity_1.0.2.zip`. El proyecto de ejemplo se identifica como `UnityThirdPersonAuthTest`.
</Note>
...
<Card title="UnityThirdPersonAuthTest" icon="flask">
  ...
  El proyecto de ejemplo está previsto para Unity 6 o una versión compatible con el package. El enlace de descarga todavía no existe en el documento fuente.
</Card>
