...
2. El launcher inicia el ejecutable con `--launchToken`.
3. El plugin lee el token desde los argumentos del proceso.
4. El plugin envía el token al endpoint de validación.
...
sequenceDiagram
    ...
    P->>P: Lee el argumento --launchToken
    P->>B: POST { token }
...
<CardGroup cols={2}>
  <Card title="Instalación desde Fab" icon="store">
    ...
    La distribución mediante Fab todavía no está habilitada. Utiliza la instalación manual.
  </Card>

  <Card title="Proyecto de ejemplo para UE 5.5" icon="folder-open">
    <Badge color="gray">Descarga todavía no disponible</Badge>

    El documento describe `GameLauncherClient`, pero no incluye una URL pública de descarga.
  </Card>
</CardGroup>

### Paquetes directos del plugin

El documento todavía no entrega enlaces reales para los paquetes por versión. Se mantienen marcados como no disponibles hasta que las URLs oficiales sean publicadas.
...
Launcher Forge genera el token cuando el jugador inicia el juego.

No debes:

- Guardarlo dentro del proyecto.
- Hardcodearlo en Blueprint.
- Hardcodearlo en C++.
- Almacenarlo en archivos de configuración.
- Mostrarlo en la interfaz.
- Imprimirlo completo en logs de producción.

El launcher entrega el token mediante:

```text
--launchToken=TOKEN
```

No se requieren argumentos adicionales para esta integración.
...
Ya no expone como campos separados:
...
Antes de compilar y vincular el juego con un launcher, puedes confirmar que la integración fue inicializada revisando el Output Log.
...
<Info>
  Este resultado confirma que el plugin y la conexión fueron inicializados. No confirma una autenticación válida, porque el editor no recibió un token generado por el launcher.
</Info>
...
5. Un launch token generado durante ese inicio.
...
<Check>
  La conexión es válida cuando el token generado por el launcher es leído por el juego, aceptado por el backend y devuelto mediante `On Success`.
</Check>
...
| `ULauncherForgeAuthSubsystem` | Guarda el token leído desde `--launchToken` y almacena el usuario después de una validación exitosa. |
...
<AccordionGroup>
  ...
  <Accordion title="El backend rechaza el token">
    Inicia nuevamente el juego desde Launcher Forge para generar un token nuevo y comprueba que el body enviado sea:
...
- El request utiliza `{ "token": "TOKEN" }`.
- La prueba final se realizó desde una build vinculada al launcher.
